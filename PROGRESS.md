...
Supabase free-tier проект был запаузен несмотря на PR #24. Причина: Mon+Thu давало максимальный промежуток 4 дня — слишком близко к 7-дневному порогу при позднем деплое или пропущенном окне. Проект запаузился до того, как cron успел сработать.

Решение: переключил расписание с `0 0 * * 1,4` на `0 12 * * *` (ежедневно в 12:00 UTC). 6× больший запас. Проект вручную восстановлен через Supabase Dashboard.
